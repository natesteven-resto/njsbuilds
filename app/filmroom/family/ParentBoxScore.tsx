 ...
 return <section aria-label="Box score" className="space-y-3">
  ...
  <div role="table" aria-label={`${group} box score`} className="text-xs">
   ...
   {rows.map(row=>{const stats=boxScore(row.entries);return <div key={row.id} role="rowgroup" className={'border-b border-white/10 '+(row.id==='team'?'bg-[#c66a3e]/10':'')}>
    <div role="row"><div role="rowheader" aria-colspan={6}><button aria-expanded={expanded===row.id} aria-label={`Show box score details for ${row.label}`} onClick={()=>setExpanded(expanded===row.id?null:row.id)} className="min-h-11 w-full break-words px-2 pt-2 text-left font-semibold hover:text-[#e49269]">{expanded===row.id?'▾':'▸'} {row.label}</button></div></div>
