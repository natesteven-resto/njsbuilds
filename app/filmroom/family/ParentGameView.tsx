...
export function ParentGameView({game, onBack}:{game:SharedGame; onBack:()=>void}) {
 ...
 const [playing,setPlaying]=useState(false),[position,setPosition]=useState(0),[duration,setDuration]=useState(0),[muted,setMuted]=useState(false)
 ...
 useEffect(()=>{
  ...
  if(!game.stats){setEntries([]);setRoster([]);setTab('clips')}
  let cancelled=false
  async function refresh(){
   await Promise.all([
    ...
    game.stats && fetch('/api/filmroom/family?view=box-score&game_id='+game.id,{cache:'no-store'}).then(async r=>{if(!r.ok)throw Error('Shared stats are unavailable.');return r.json()}).then(data=>{if(!cancelled){setEntries(data.entries);setRoster(data.players);setStatsError('')}}).catch(e=>{if(!cancelled){setEntries([]);setRoster([]);setStatsError(e.message)}})
 ...
 function togglePlay(){const v=video.current;if(!v)return;if(v.paused)void v.play().catch(()=>setControlError('Press play to retry.'));else v.pause()}
 ...
 const playerIds=Array.from(new Set([...roster.map(p=>p.id),...entries.filter(e=>e.player_id).map(e=>e.player_id)]))
 ...
 return <section aria-label="Shared game workspace" className="space-y-4">
  ...
  <div ref={workspace} className="grid min-h-0 gap-4 bg-[#181917] lg:grid-cols-[minmax(0,1fr)_360px] [&:fullscreen]:overflow-y-auto [&:fullscreen]:p-4">
   <div className="min-w-0 space-y-3">
    {canPlay?<><video ref={video} playsInline preload="metadata" tabIndex={0} aria-label="Game video. Click or press Space to play or pause." onClick={togglePlay} onKeyDown={e=>{if(e.key===' '){e.preventDefault();togglePlay()}}} onTimeUpdate={()=>{setPosition(video.current?.currentTime||0);enforceClip()}} onPlay={()=>{setPlaying(true);resumeClip()}} onPause={()=>setPlaying(false)} onVolumeChange={()=>setMuted(!!video.current?.muted)} onLoadedMetadata={()=>{if(video.current){video.current.playbackRate=speed;setDuration(video.current.duration)}}} className="aspect-video max-h-[78vh] w-full rounded-lg bg-black"/>
     <div className="rounded-lg border border-white/10 bg-[#13161a] p-3"><input aria-label="Video timeline" type="range" min={0} max={Number.isFinite(duration)?duration:0} step={.1} value={position} onChange={e=>{if(video.current)video.current.currentTime=Number(e.target.value)}} className="mb-3 w-full accent-[#c66a3e]"/><div className="flex flex-wrap items-center gap-2"><button className={button+' bg-[#c66a3e] font-semibold'} onClick={togglePlay}>{playing?'Pause':'Play'}</button>
      ...
      <button className={button} onClick={()=>{if(video.current)video.current.currentTime=Math.min(Number.isFinite(duration)?duration:Infinity,video.current.currentTime+5)}}>+5s</button><span className="text-xs tabular-nums text-[#c9c3b8]">{clock(position*1000)} / {clock((Number.isFinite(duration)?duration:0)*1000)}</span><button className={button} onClick={()=>{if(video.current)video.current.muted=!video.current.muted}}>{muted?'Unmute':'Mute'}</button><label className="flex min-h-11 items-center gap-2 px-2 text-sm">Speed<select aria-label="Playback speed" value={speed} onChange={e=>setSpeed(Number(e.target.value))} className="rounded bg-[#252622] p-2">{[.5,.75,1,1.25,1.5,2].map(s=><option key={s} value={s}>{s}×</option>)}</select></label>
      <button className={button} onClick={()=>{if(document.fullscreenElement){void document.exitFullscreen()}else if(workspace.current?.requestFullscreen){void workspace.current.requestFullscreen().catch(()=>setControlError('Fullscreen is unavailable in this browser. Try Safari’s fullscreen command.'))}else {const v=video.current as (HTMLVideoElement&{webkitEnterFullscreen?:()=>void})|null;if(v?.webkitEnterFullscreen)v.webkitEnterFullscreen();else setControlError('Fullscreen is unavailable in this browser.')}}}>Fullscreen</button>
      <PlaybackQualityControl gameId={game.id} quality={quality} onChange={setQuality}/>
     </div></div>{activeClip?<div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-[#c66a3e]/40 p-3 text-sm"><span>Clip: <strong>{activeClip.title||'Untitled clip'}</strong> · Stops at {clock(activeClip.end_time_ms)}</span><button className={button} onClick={()=>{setActiveClip(null);void video.current?.play().catch(()=>{})}}>Continue full game</button></div>:<p className="text-xs text-[#c9c3b8]">Watching full game. Choose a shared clip in Clips to watch a teaching moment.</p>}
   ...
   <aside className="min-w-0 rounded-lg border border-white/10 bg-[#1e201d]">
    ...
    <div id="parent-panel" role="tabpanel" aria-labelledby={'parent-tab-'+tab} className="max-h-[80vh] space-y-4 overflow-y-auto overscroll-contain p-4 [scrollbar-color:#55574f_#1e201d] [scrollbar-width:thin]">
     ...
     {tab==='clips'&&<><p className="text-xs text-[#c9c3b8]">{clips.length} shared {clips.length===1?'clip':'clips'} · From your coach</p>{clipError&&<p role="alert" className="text-red-300">{clipError}</p>}{!loading&&!clips.length&&!clipError&&<p className="py-6 text-sm text-[#c9c3b8]">No clips shared yet. Your coach’s clips will appear here unless marked private.</p>}{clips.map(c=><button key={c.id} disabled={!canPlay} onClick={()=>playAt(c.start_time_ms,c)} aria-pressed={activeClip?.id===c.id} className={'w-full rounded-xl border p-4 text-left disabled:opacity-50 '+(activeClip?.id===c.id?'border-[#c66a3e] bg-[#c66a3e]/10':'border-white/10 bg-[#252722] hover:border-white/30')}><span className="block font-semibold">▶ {c.title||'Untitled clip'}</span><span className="mt-2 block text-xs text-[#c9c3b8]">{clock(c.start_time_ms)} – {clock(c.end_time_ms)}{c.category?' · '+c.category:''}</span></button>)}</>}
     {tab!=='clips'&&<><label className="block text-xs text-[#c9c3b8]">Player<select value={player} onChange={e=>setPlayer(e.target.value)} className="mt-2 min-h-11 w-full rounded border border-white/15 bg-[#252722] px-3 text-sm"><option value="all">All players</option>{playerIds.map(id=>{const p=roster.find(p=>p.id===id),e=entries.find(e=>e.player_id===id);return <option key={id} value={id}>#{p?.number??e?.player_number??'?'} {p?.name??e?.player_name}</option>})}</select></label>{statsError&&<p role="alert" className="text-red-300">{statsError}</p>}</>}
     {tab==='stats'&&!loading&&!statsError&&<ParentBoxScore entries={entries} players={roster} playerId={player} canPlay={canPlay} onSeek={ms=>playAt(ms)}/>}
