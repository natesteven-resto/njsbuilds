...
export function ToolbarMenu({label,children}:{label:string;children:ReactNode}){
 ...
 return <details ref={ref} className="relative" onKeyDown={e=>{if(e.key==='Escape'){e.stopPropagation();ref.current?.removeAttribute('open');ref.current?.querySelector('summary')?.focus()}}}>
  ...
  <div className="absolute bottom-full right-0 mb-2 z-50 w-64 max-w-[85vw] max-h-[60vh] overflow-y-auto rounded-xl border border-white/20 bg-[#20211e] p-3 shadow-xl space-y-3 text-xs text-white/80" role="group" aria-label={label}>{children}</div>
 </details>
}
