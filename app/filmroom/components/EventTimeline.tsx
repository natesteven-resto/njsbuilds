...
export function EventTimeline({clips,stats,durationMs,currentMs,selectedId,onSeek,onClip,leadIn=5,actions,settings}:{clips:Clip[];stats:FilmStat[];durationMs:number;currentMs:number;selectedId:string|null;onSeek:(ms:number)=>void;onClip:(clip:Clip)=>void;leadIn?:number;actions?:ReactNode;settings?:ReactNode}) {
 ...
 <div className="flex flex-wrap justify-between gap-2 text-xs text-white/75 mb-2"><span>Film timeline · {clips.length} clip{clips.length===1?'':'s'} · {stats.length} stats</span><div className="flex items-center gap-3"><span>{time(durationMs)}</span>{actions}</div></div>
 {settings}
 <p className="text-xs text-white/60 mb-3">Select an event to play from {leadIn} seconds before it. Use Events above to filter.</p>
