...
export function PlaybackQualityControl({gameId,sourceKey='',quality,onChange}:{gameId:string;sourceKey?:string;quality:PlaybackQuality;onChange:(q:PlaybackQuality)=>void}){
 ...
 useEffect(()=>{setStatus(null);setError('')},[gameId,sourceKey])
 useEffect(()=>{
 ...
 return <div className="flex flex-wrap items-center gap-2 text-xs">
  ...
  {busy&&<span role="status" className="text-white/60">Preparing Auto…</span>}
  {!busy&&status&&['submitting','processing'].includes(status.state)&&<span role="status" className="text-white/60">Preparing Auto{status.progress>0?` · ${status.progress}%`:'…'}</span>}
