...
function PlayerCard({ player, onDelete }: { player: Player; onDelete: (id: string) => void }) {
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3 rounded-2xl bg-white/3 border border-white/8 hover:bg-white/5 transition-colors group">
      <div className="w-10 h-10 rounded-xl bg-[#c66a3e]/15 border border-[#c66a3e]/25 flex items-center justify-center shrink-0">
        <span className="font-bold text-[#e79568] text-sm">{player.number ?? '—'}</span>
      </div>
      ...
      <Link href={`/filmroom/player/${player.id}`}
        className="px-3 py-1.5 text-xs text-white/60 hover:text-white/70 hover:bg-white/6 rounded-xl transition-all opacity-0 group-hover:opacity-100">
        View Portal →
      </Link>
      <button onClick={handleDelete} disabled={deleting}
        aria-label={`Remove ${player.name} (#${player.number ?? '?'})`}
        className="p-1.5 rounded-lg text-white/60 hover:text-red-400 hover:bg-red-500/10 transition-all opacity-0 group-hover:opacity-100 disabled:opacity-40">
...
function AddPlayerModal({ teamId, onClose, onAdd }: {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-[#20211e] border border-white/10 rounded-2xl w-full max-w-sm p-5" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-semibold text-sm">Add Player</h2>
          <button onClick={onClose} aria-label="Close" className="p-1.5 rounded-lg hover:bg-white/8 text-white/60 hover:text-white transition-all">
            <X className="w-4 h-4" aria-hidden />
          </button>
        </div>
        <form onSubmit={submit} className="space-y-3">
          <div>
            <label htmlFor="player-name" className="block text-xs text-white/60 mb-1">Full Name *</label>
            <input id="player-name" required type="text" placeholder="Jordan Williams"
              value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
              className="w-full bg-black/30 border border-white/10 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-[#c66a3e] placeholder-white/20" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label htmlFor="player-number" className="block text-xs text-white/60 mb-1">Jersey #</label>
              <div className="relative">
                <Hash className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-white/60" aria-hidden />
                <input id="player-number" type="number" min="0" max="99" placeholder="23"
                  ...
                  className="w-full bg-black/30 border border-white/10 rounded-xl pl-8 pr-3 py-2 text-sm focus:outline-none focus:border-[#c66a3e] placeholder-white/20" />
              </div>
            </div>
            <div>
              <label htmlFor="player-position" className="block text-xs text-white/60 mb-1">Position</label>
              <select id="player-position" value={form.position} onChange={e => setForm(f => ({ ...f, position: e.target.value }))}
                className="w-full bg-black/30 border border-white/10 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-[#c66a3e] text-white/80">
                <option value="">—</option>
          ...
          <div>
            <label htmlFor="player-email" className="block text-xs text-white/60 mb-1">Parent Email <span className="text-white/25">(for portal access)</span></label>
            <div className="relative">
              <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-white/60" aria-hidden />
          ...
          <div className="flex gap-2 pt-1">
            ...
            <button type="submit" disabled={loading}
              className="flex-1 py-2.5 rounded-xl bg-[#c66a3e] hover:bg-[#dc8253] text-[#181917] text-xs font-medium transition-colors flex items-center justify-center gap-1.5 disabled:opacity-50">
...
export default function RosterPage() {
  ...
  return (
    <div className="cs min-h-screen bg-[#181917] text-[#eee9df]">
      <header className="border-b border-white/8 sticky top-0 z-40 bg-[#181917]">
        <div className="max-w-3xl mx-auto px-4 h-14 flex items-center gap-3">
          <Link href="/filmroom" className="p-1.5 rounded-lg hover:bg-white/8 text-white/60 hover:text-white transition-all" aria-label="Back to Film Room">
            <ChevronLeft className="w-4 h-4" aria-hidden />
          </Link>
          <div className="flex items-center gap-2">
            <Users className="w-4 h-4 text-[#e79568]" aria-hidden />
            <span className="font-semibold text-sm">Roster</span>
            {teamName !== 'Roster' && <><span className="text-white/60 text-sm">·</span><span className="text-white/60 text-sm">{teamName}</span></>}
          </div>
          <div className="flex-1" />
          {!loading && !error && <span className="text-xs text-white/60 mr-2">{players.length} player{players.length !== 1 ? 's' : ''}</span>}
          {teamId && (
            <button onClick={() => setShowAdd(true)}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-[#c66a3e] hover:bg-[#dc8253] text-[#181917] text-xs font-medium transition-colors">
        ...
        {!loading && !error && players.length === 0 && (
          <div className="text-center py-20">
            <div className="w-16 h-16 rounded-2xl bg-white/4 flex items-center justify-center mx-auto mb-4">
              <Users className="w-8 h-8 text-white/60" aria-hidden />
            </div>
            <h3 className="font-semibold text-white/60 mb-2">Empty roster</h3>
            <p className="text-sm text-white/60 mb-6">Add your players to start tagging them in clips and entering stats.</p>
            {teamId && (
              <button onClick={() => setShowAdd(true)}
                className="flex items-center gap-2 px-5 py-2.5 bg-[#c66a3e] hover:bg-[#dc8253] text-[#181917] rounded-xl text-sm font-medium transition-colors mx-auto">
        ...
        {!loading && !error && players.length > 0 && (
          <div className="space-y-6">
            {positionGroups.map(({ pos, players: grp }) => (
              <div key={pos}>
                <h3 className="text-xs font-semibold text-white/60 uppercase tracking-wider mb-2">{pos}</h3>
            ...
            {unpositioned.length > 0 && (
              <div>
                <h3 className="text-xs font-semibold text-white/60 uppercase tracking-wider mb-2">No Position</h3>
                <div className="space-y-2">
