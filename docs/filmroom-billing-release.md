...
3. Apply migrations 015 and 016 with enforcement OFF to dedicated Film Room Supabase only. Verify authenticated ownership grants in the deployed database.
...
## Full isolated local integration completed
Set up a dedicated local Supabase project and MinIO storage, with no customer data copied. Real Supabase authentication, middleware, RLS, billing routes, and multipart endpoints were exercised together. Two accounts received private demos; unpaid creation was rejected; paid game creation, cross-account denial, resume, two-part upload (104,858,624 bytes), exact quota accounting, and idempotent completion passed. Seeded past-due state blocked new games while existing library reads remained available.

Fresh-schema testing found service_role lacked base-table access. Added migration 016 with explicit DML grants on named Film Room tables only; browser grants remain unchanged. The disposable PostgreSQL suite now checks server-role access and passed with zero failures.

Official Stripe CLI forwarded real sandbox payment/cancellation webhooks into the local Next.js app. PostgreSQL state changed automatically to active then canceled. Two replays of an old active payload left it canceled. Browser checkout from the canceled account's Subscribe button completed a $25 test resubscription and returned to “Your plan is active.”

Sandbox customer and subscriptions removed after testing; local generated accounts and password fixture removed. No live billing or deployment. Cloudflare R2/browser CORS validation still remains; local MinIO does not prove provider-specific behavior. Full local evidence and setup notes are in the coordinating task's `outputs/filmroom-local-integration.md`.
