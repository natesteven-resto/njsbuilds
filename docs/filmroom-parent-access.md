...
## Full parent box score (2026-09-18)

Migration 021 adds a stats-permission-checked box score RPC returning event data plus only player ID/name/number for the game's owned team. This includes zero-stat players without exposing private roster metadata. The parent Stats tab now provides Overview, Shooting, and Defense columns, team totals, opponent totals when recorded, and expandable complete player stats with play links. Six columns per view fit the sidebar without horizontal scrolling. Shot chart and player filtering use the same authorized response.

Validated scoring/attempts/percentage calculations, zero-stat rows, safe roster projection and film-only denial using local PostgreSQL tests, plus browser layout and five-second lead-in links. Safari's existing parent session is now accepted and can view Tryouts; the old six-stat cards were observed there before this update.
